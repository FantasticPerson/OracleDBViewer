...
var DB_OWNER=oracleDb.getting().dbowner;
//表名
var _prev = '';
var tableNames = {
	tb_yjsg:DB_OWNER+"GX_HB_NIGHT_WORK"//夜间施工
	,tb_hbck:DB_OWNER+"HB_JDC_HB_WINDOW"//环保窗口
	,tb_hbjcz:DB_OWNER+"HB_JDC_CHECK_SITE"//环保检测站
	,tb_gkyjk:DB_OWNER+"HB_ZDYZX_QY"//国控源监控//重点源在线监控企业名单
	,tb_fqjc:DB_OWNER+"HB_T_WRY_JCZF_FQ"//废气检测结果
	,tb_fsjc:DB_OWNER+"HB_T_WRY_ZDJC_FS"//废水检测结果
	,vw_fq:DB_OWNER+"V_NEW_ZDY_FQ"//废气
	,vw_fs:DB_OWNER+"V_NEW_ZDY_FS"//废水
	,tb_waterFacName:DB_OWNER+"HBJ_T_HJZL_OBJSTATION_WAT"//水厂
//	1007	远古水厂
//	1006	城南水厂
//	1004	林山
	,tb_waterDayData:DB_OWNER+"HBJ_T_HJZL_WAT_DAY"//水厂当日数据
	,tb_waterf:DB_OWNER+"HB_WARTERFACTORY"
	,tb_waters:DB_OWNER+"HB_WARTERSOURCE"
	,tb_watersmm:DB_OWNER+"HB_WARTERSOURCE_MONITOR_MONTHLY"
	,vw_wmm:DB_OWNER+"V_WS_MONITOR_MONTHLY"
	,vw_moto:DB_OWNER+"V_VEHICLE_LABEL_LIST"//机动车
	,tb_motonj:DB_OWNER+"HBJ_T_JDC_ASM_RESULT"//机动车年检详情
	,tb_watermm2:DB_OWNER+"HB_WATERSOURCE_MONITOR_MONTHLY"//水源
	,tb_waterfs:DB_OWNER+"HB_WATERFACTORY"//水厂
}
...
var Sqls = {
	...
	,motoinfo:"select * from "+tableNames.vw_moto+" t where t.VEHICLE_TAG_NO='{$vn}' and substr(t.MOTO_NO,-6,6)='{$mn}'"
	,motonj:"select * from "+tableNames.tb_motonj+" t where t.vehicle_sn='{$sn}' order by inspection_date desc"
	,waterfs:"select * from "+tableNames.tb_waterfs
	,watermm2:"select * from "+tableNames.tb_watermm2 + " t where t.WS_ID='{$wsid}'"
}
...
Main.prototype = {
	...
	apiList:function(){
		...
		//夜间施工
		this.app.get(nginx_proxy+'/nightWork', function(req, res) {
			that.conn();
		...
		//环保监测站
		this.app.get(nginx_proxy+'/hbjcz', function(req, res) {
			that.conn();
		...
		//环保窗口
		this.app.get(nginx_proxy+'/hbWindow', function(req, res) {
			that.conn();
			if(req.query.type){
		...
		//国控源监控
		this.app.get(nginx_proxy+'/gkyjc', function(req, res) {
			that.conn();
			if(req.query.name){
		...
		//废水
		this.app.get(nginx_proxy+'/fs', function(req, res) {
			that.conn();
			if(req.query.name){
		...
		//废气
		this.app.get(nginx_proxy+'/fq', function(req, res) {
			that.conn();
			if(req.query.name){
		...
		//水厂当日详情
		this.app.get(nginx_proxy+'/waterday', function(req, res) {
			that.conn();
			if(req.query.id){
				...
				oracleDb.query(sql, [], function(rows, msg){
					...
					for(var i=0,l=rows.length;i<l;i++){
						arr.push({
							...
							"ph":rows[i][2] ? rows[i][2].toFixed(2) : rows[i][2],
							"shuiwen":rows[i][3],
		...
		//机动车
		//?vn=苏A67E34&mn=34597470256S1
		//select * from ZHNJ.V_VEHICLE_LABEL_LIST where VEHICLE_TAG_NO = vn and MOTO_NO = mn
		this.app.get(nginx_proxy+'/motoinfo', function(req, res) {
			that.conn();
			var sql = Sqls.motoinfo;
			if(req.query.vn){
				sql = sql.replace('{$vn}',req.query.vn);
			}
			if(req.query.mn){
				sql = sql.replace('{$mn}',req.query.mn);
			}
			...
			if(typeof(sql)!=="undefined"){
				oracleDb.query(sql, [], function(rows, msg){
					...
					"data":[
						...
						4"34597470256S1",
						5"绿标",
						6"2016-06-30T16:00:00.000Z",
						7"1991-12-31T16:00:00.000Z",
						8//6,
						9"1991-12-31T16:00:00.000Z",
						10"00029000005357753",
						11"2016-02-29T16:00:00.000Z",
						12"00029"]
					]
					}*/
					if(rows){
						for(var i=0,l=rows.length;i<l;i++){
							arr.push({
								...
								"cphm":rows[i][1],
								"pzlx":rows[i][2],
								"hfrq":(new Date(rows[i][11])).pattern("yyyy-MM-dd"),
								"bzlx":rows[i][5],
								"dqsj":(new Date(rows[i][6])).pattern("yyyy-MM-dd"),
								"mn":rows[i][4]
							});
						}
					}
		...
		//机动车年检详情
		this.app.get(nginx_proxy+'/motonj', function(req, res) {
			that.conn();
			var sql = Sqls.motonj;
			if(req.query.sn){
				sql = sql.replace('{$sn}',req.query.sn);
			}
			...
			if(typeof(sql)!=="undefined"){
				oracleDb.query(sql, [], function(rows, msg){
					...
					if(rows){
						for(var i=0,l=rows.length;i<l;i++){
							arr.push({
								"jcrq":(new Date(rows[i][1])).pattern("yyyy-MM-dd"),
								"jczt":rows[i][2],
								"co25_sz":rows[i][3],
								"hc25_sz":rows[i][4],
								"no25_sz":rows[i][5],
								...
								"hc40_sz":rows[i][8],
								"no40_sz":rows[i][9],
								"result":rows[i][11].replace('        ','').replace(' ','')
							});
						}
					}
					returnobj.data = arr;
		...
		//水厂的水源地检测列表
		this.app.get(nginx_proxy+'/waterfs', function(req, res) {
			that.conn();

			var sql = Sqls.waterfs;
			function promiseGetIsPastById(id){
				return new Promise(function(resolve,reject){
					var sql2 = Sqls.watermm2;
					sql2 = sql2.replace('{$wsid}',id);
					if(typeof(sql2)!=="undefined"){
						oracleDb.query(sql2,[],function(rows,msg){
							console.log(rows);
							if(msg != null){
								console.log(msg);
								return resolve(msg);
							}
							if(rows && rows.length > 0 && rows[0][7]){
								console.log('id:'+id+' found');
								return resolve(rows[0][7]);
							} else {
			...
			if(typeof(sql)!=="undefined"){
				oracleDb.query(sql, [], function(rows, msg){
					...
					if(rows){
						let promiseArr = [];
						for(var i=0,l=rows.length;i<l;i++){
							promiseArr.push(promiseGetIsPastById(rows[i][5]));
							arr.push({
								"wsid":rows[i][5],
								"name":rows[i][1],
								"weight":rows[i][2]
		...
		//水厂的水源地
		this.app.get(nginx_proxy+'/watermm2', function(req, res) {
			that.conn();
			var sql = Sqls.watermm2;
			if(req.query.id){
				sql = sql.replace('{$wsid}',req.query.id);
			}
		...
		//排名
		this.app.get(nginx_proxy+'/map', function(req, res) {
		...
		this.app.get(nginx_proxy+'/order', function(req, res) {
		...
		this.app.get(nginx_proxy+'/order2', function(req, res) {
